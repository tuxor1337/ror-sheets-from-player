...
import TuneBodyGroove from './TuneBodyGroove.vue'
import TuneBodyBreak from './TuneBodyBreak.vue'
import TuneBodyText from './TuneBodyText.vue'
...
const sizing = computed(() => {
  ...
  result['n_subbeat_cells'] = result['upbeats'] + result['subbeats_per_row'] + result['afterbeats']
  return result
})

const is_tune_type = computed(() => {
  return props.name.toLowerCase().indexOf('tune') >= 0 || pattern.value.separate_instruments
})
</script>
...
<template>
  <table>
    ...
    <TuneBodyGroove v-if="is_tune_type" :name="name" :pattern="pattern" :sizing="sizing" />
    <TuneBodyText v-else-if="pattern?.text" :name="name" :pattern="pattern" :sizing="sizing" />
    <TuneBodyBreak v-else :pattern="pattern" :sizing="sizing" />
