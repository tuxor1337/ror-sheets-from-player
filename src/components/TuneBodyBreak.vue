...
const rem_indent = computed(() => props.pattern?.remarks_indent ?? false)
const numRemarks = computed(() => (props.pattern?.remarks ? props.pattern.remarks.length : 0))
...
function _untangle_afterbeat(notes_first_iter, notes_second_iter) {
  ...
  for (let i = 0; i < notes_first_iter.length; i++) {
    const char1 = notes_first_iter.charAt(i)
    const char2 = notes_second_iter.charAt(i)
...
function _render_lines(data, i_notes) {
  const upbeat = data?.upbeat ?? 0
  const repeat_with_afterbeats = data?.repeat_with_afterbeats ?? []
  const notes_override =
    !!data?.notes_override && data.notes_override.length > i_notes
      ? data.notes_override[i_notes]
      : {}
  const notes = data['notes'][i_notes]
  const n_lines =
    Math.ceil((notes.length - upbeat) / props.sizing['subbeats_per_row']) + (upbeat > 0 ? 1 : 0)
  const n_initlines_repeated =
    props.sizing['upbeats'] < upbeat ? 0 : _count_repeated_initlines(notes, upbeat)
  ...
  const lines = offsets.map((offset, i_line) => {
    return notes.substr(
      offset,
      i_line + 1 < n_lines ? offsets[i_line + 1] - offset : props.sizing['subbeats_per_row'],
    )
  })

  const result = []
  for (let i_line = 0; i_line < n_lines; i_line++) {
    const offset = offsets[i_line]
    const length =
      i_line + 1 < n_lines ? offsets[i_line + 1] - offset : props.sizing['subbeats_per_row']
    const override = {}
    ...
    const preset = repeat_with_afterbeats.find(([i]) => i == i_line + 1)
    let afterbeats = 0
    if (i_line == 0 && n_initlines_repeated > 0) {
      ...
      afterbeats = preset[3]
    } else if (n_overrides == 0 && !data?.nosqueeze) {
...
function breakLineToCells(
  ...
  if (i_line == 0) {
    ...
    if (!!preamble && upbeat > 0) {
      const cell = row.cells.filter((c) => !!c.colSpan)[0]
      cell.classList.push('text', 'remark')
      cell.content = preamble
    }
  }

  let note_cells = row.cells.filter((c) => c.classList.indexOf('note') >= 0)
  if (i_line != 0 || upbeat > props.sizing['upbeats']) {
    note_cells = note_cells.filter((c) => c.classList.indexOf('upbeat') < 0)
  }
  note_cells[0].classList.push('line_start')
  note_cells[note_cells.length - 1].classList.push('line_end')
  const n_note_cells = note_cells.length

  if (i_line == 1 && upbeat > props.sizing['upbeats']) {
    Array.from(note_cells).forEach((cell, i_cell) => {
      if (i_cell < n_note_cells - upbeat) {
        cell.classList.push('break_start')
      }
    })
  }
  ...
  if (aside) {
    if (i_line > 0) {
      ...
      cell.rowSpan = n_lines
      cell.classList.push('text', 'aside')
      cell.content = aside
    }
  }
  if (aside_lines) {
    ...
    for (const aside_lstart in aside_lines) {
      ...
      if (aside_lstart <= i_line + 1 && i_line + 1 <= aside_lstart + aside_llen - 1) {
        if (i_line + 1 > aside_lstart) {
          // row.cells.splice(-1, 1);
        } else {
          cell.rowSpan = aside_llen
          cell.classList.push('text', 'aside_lines')
          cell.content = aside_text
        }
      }
    }
  }
  if (!!subtitle && i_line > 0) {
    if (i_line > 1) {
      ...
      cell.rowSpan = n_lines - 1 + n_extra_lines
      cell.classList.push('remark')
...
const tuneRows = computed(() => {
  ...
  const rows = lines_render.value.map(({ str_row, notation, override }, i_line) =>
    breakLineToCells(i_line, n_lines_render, { str_row, notation, override }, props.pattern),
  )
  if (n_lines_render > 1) {
    ...
    for (let i_subbeat = 0; i_subbeat < props.sizing['subbeats_per_row']; i_subbeat++) {
      ...
      if (last_line < n_lines_render - 1) {
        const cell = rows[last_line]
          .filter((c) => c.classList.indexOf('note') >= 0)
          .filter((c) => c.classList.indexOf('upbeat') < 0)[i_subbeat]
        cell.classList.push('break_end')
      }
    }
  }
  return rows
...
<template>
  <tbody v-if="pattern?.preamble && pattern.upbeat == 0">
    <tr>
      ...
      <td class="text remark" :colspan="sizing.ncols - 2 - sizing.upbeats">
        {{ pattern.preamble }}
      </td>
    </tr>
  </tbody>
  ...
  <tbody v-if="n_extra_lines > 0">
    <tr v-for="(_, i_line) in n_extra_lines" :key="i_line">
      ...
      <template v-if="numRemarks > i_line">
        <td class="empty" :colspan="1 + rem_indent"></td>
        <td class="text remark" :colspan="sizing.ncols - 2 - rem_indent">
          {{ pattern.remarks[i_line] }}
        </td>
      </template>
      <template v-else>
  ...
  <tbody v-if="numRemarks > n_extra_lines">
    <tr v-for="(remark, i_rem) in pattern.remarks.slice(n_extra_lines)" :key="i_rem">
      <template v-if="rem_indent !== false">
        <td class="empty" :colspan="2 + sizing.upbeats + rem_indent"></td>
      </template>
      <td
        :colspan="sizing.ncols - (rem_indent !== false ? 2 + sizing.upbeats + rem_indent : 0)"
        class="text remark"
      >
        {{ remark }}
      </td>
  ...
  <tbody v-if="pattern?.memory_aid">
    <tr class="empty" v-if="numRemarks >= n_extra_lines">
